...
export async function initEbooks(container) {
  ...
  try {
    ...
    const createBookCard = (book) => {
      ...
      // Read online button
      if (book.pdf_url) {
        const readBtn = el("a", "read-btn", { text: "Read Online", href: book.pdf_url, target: "_blank" });
        btnContainer.appendChild(readBtn);

        // Download button
        const downloadBtn = el("a", "download-btn", { text: "Download PDF", href: `/api/ebooks/download/${book.id}` });
