...
export async function initSermons(container) {
  ...
  try {
    ...
    for (const sermon of sermons) {
      ...
      /** Lazy-load video */
      const setupVideo = () => {
        if (window.Hls && sermon.hls_url && Hls.isSupported()) {
      ...
      const lazyObserver = new IntersectionObserver(entries => {
        entries.forEach(e => { if(e.isIntersecting){ setupVideo(); lazyObserver.unobserve(video); }});
      ...
      async function refreshLikes() {
        try { const res = await api.get(`/likes/count?type=sermon&sermon_id=${sermon.id}`);
